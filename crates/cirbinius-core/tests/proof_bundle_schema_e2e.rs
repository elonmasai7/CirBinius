...
#[test]
fn emitted_proof_bundle_matches_v1_schema() {
    let workspace_root = common::workspace_root();
    let temp_dir = common::temp_dir("proof-bundle-schema-e2e");
    ...
    let outcome = dispatch(
        CommandAction::Prove(ProveArgs {
            ...
            precheck_report_path: Some(temp_dir.join("prove_precheck_report.json")),
            precheck_only: true,
    ...
    let proof_bundle_path = outcome
        .artifact_path
        .expect("prove should return proof bundle path");
    let proof_bundle_json =
        fs::read_to_string(proof_bundle_path).expect("proof bundle file should be readable");
    let proof_bundle_value: Value =
        serde_json::from_str(&proof_bundle_json).expect("proof bundle should parse as json");

    let schema_path = workspace_root.join("docs/contracts/proof-bundle-v1.schema.json");
    let schema_text = fs::read_to_string(schema_path).expect("proof bundle schema should exist");
    ...
    if let Err(errors) = compiled.validate(&proof_bundle_value) {
        let messages = errors.map(|err| err.to_string()).collect::<Vec<_>>();
        panic!(
            "emitted proof bundle violates v1 schema: {}",
            messages.join(" | ")
        );
    }
}
